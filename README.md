...
- Lock a file to prevent changes with Ctrl+Alt+K (Cmd+Opt+K on Mac). All features below except for the unlock feature will lock the file to prevent unwanted changes.
...
- Read the clipboard for a locator in chapter:line:column format and go to that location: Ctrl+Alt+A (Cmd+Opt+H on Mac). If the locator is a range (e.g., 01:1:1-01:19:1), the entire range will be highlighted.
